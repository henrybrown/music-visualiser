 ...
 *
 * Reads directly from the animation engine - no registration or syncing needed.
...
export const DevToolsPanel: React.FC<{
  ...
  // Read data directly from engine
  const getEntityGroups = (): Map<string, EntityData> => {
    const engineData = (engine as any)._devtoolsData;
    if (!engineData) return new Map();

    const entities = new Map<string, EntityData>();

    engineData.elementRegistry.forEach((data: any, element: HTMLElement) => {
      ...
      const animKey = `${entityId}-${elementId}`;
      const isAnimating = Array.from(engineData.runningAnimations.keys()).some((key: string) =>
        key.startsWith(animKey),
      );
  ...
  return (
    <div style={styles.panel}>
      ...
      <div style={styles.stats}>
        ...
        <span>⚙️ Registered: {debugInfo.registrySize}</span>
        <span>▶ Running: {debugInfo.runningAnimations}</span>
      </div>
      ...
      <div style={styles.content}>
        ...
        {entities.size === 0 && <div style={styles.emptyState}>No entities registered yet...</div>}
      </div>
    </div>
  );
};
      ...
      <div style={styles.header} onClick={onToggle}>
        <div style={styles.title}>
          ...
          <div style={styles.badges}>
            {displayState && (
              <span
                style={{ ...styles.badge, background: getStateColor(displayState), color: "#fff" }}
              >
                {displayState}
              </span>
            )}
